...
const AddJob = () => {
    ...
    const handleAddJob = e => {
        ...
        const emplyoerEmail = form.employer_email.value;
        const jobCategory = form.job_category.value;
        const minSelary = form.min_selary.value;
        const maxSelary = form.max_selary.value;
        const deadline = startDate;
        const jobDescription = form.job_description.value;
        const jobData = {
            jobBanner, jobTitle, jobCategory, minSelary, maxSelary, deadline, jobDescription, emplyoer: {
                email: emplyoerEmail,
                name: emplyoerName
            },
            applied_count: 0
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center pb-20">
            ...
            <form onSubmit={handleAddJob} className="md:w-[80%] flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2">
                    ...
                    <div>
                        <label className="text-lg font-medium">Selary Range </label>
                        <div className="flex gap-4 mt-2">
                            <input className="w-full p-2 rounded bg-[#1B3044] text-white" type="number" name="min_selary" placeholder="Minimum Selary" id="" />
                            <input className="w-full p-2 rounded bg-[#1B3044] text-white" type="number" name="max_selary" placeholder="Minimum Selary" id="" />
                        </div>
                    </div>
                </div>
                <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                    ...
                    <DatePicker className="w-full p-2 rounded bg-[#1B3044] text-white" selected={startDate} onChange={(date) => setStartDate(date)} />
                </div>
